...
export function chatUrl(jobId) {
  ...
  return liffId ? `https://liff.line.me/${liffId}?job=${jobId}` : null;
}
...
// แจ้งเตือนมีข้อความแชทใหม่ พร้อมปุ่มเปิดแชท (counterpartLabel = "ผู้รับงาน" หรือ "เจ้าของงาน" แล้วแต่ว่าใครเป็นผู้ส่ง)
export function buildChatNotifyMessage({ counterpartLabel, senderName, jobDetail, chatUri }) {
  return {
    type: "flex",
    altText: `${counterpartLabel} ส่งข้อความถึงคุณ`,
    contents: {
      type: "bubble",
      body: {
        type: "box",
        layout: "vertical",
        contents: [
          { type: "text", text: `💬 ${counterpartLabel} ส่งข้อความถึงคุณ`, weight: "bold", size: "lg", wrap: true },
          { type: "text", text: `ชื่อผู้ส่ง: ${senderName}`, size: "md", margin: "md" },
          { type: "text", text: `ชื่องาน: ${jobDetail}`, size: "md", margin: "sm", wrap: true },
        ],
      },
      footer: {
        type: "box",
        layout: "vertical",
        contents: [
          { type: "button", style: "primary", action: { type: "uri", label: "💬 เปิดแชท", uri: chatUri } },
        ],
      },
    },
  };
...
export function buildWelcomeMessage({ guideUrl, addFriendUrl, profileUrl } = {}) {
    ...
    contents: {
      ...
      footer: (() => {
        ...
        if (profileUrl) {
          buttons.push({
            type: "button",
            style: buttons.length ? "secondary" : "primary",
            action: { type: "uri", label: "📝 กรอกข้อมูลส่วนตัว", uri: profileUrl },
          });
        }
        if (guideUrl) {
          buttons.push({
            type: "button",
            style: buttons.length ? "link" : "primary",
