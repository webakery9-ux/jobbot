...
export async function getJobCloseDetails(jobId) {
  const { data, error } = await supabase
    .from("jobs")
    .select(
      "id, detail, wage, payment_method, status, poster:poster_id(display_name), group:group_id(group_name), claims:job_claims(claimed_at, released_at, delivery_at, delivery_note, delivery_photo_url, claimer:claimed_by(display_name, phone))"
    )
    .eq("id", jobId)
    .single();
  ...
  const currentClaim = claims.find((c) => !c.released_at && !c.delivery_at) ?? null;
  const doneClaim = claims.find((c) => c.delivery_at) ?? null;
  ...
  return {
    ...
    status: data.status,
    posterName: data.poster?.display_name ?? "-",
