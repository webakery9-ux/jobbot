...
function formatBatchLabelDate(isoDateStr) {
  ...
  const weekday = new Date(`${isoDateStr}T00:00:00+07:00`).toLocaleDateString("th-TH", {
    weekday: "short",
    timeZone: "Asia/Bangkok",
  });
...
// jobs: อาเรย์ของงานที่โพสต์สำเร็จ แต่ละอันต้องมี previewLine (3 บรรทัดสรุปย่อ คำนวณไว้แล้วตอน parse)
export function buildBatchSummaryMessage(jobs, batchCode, batchLabelDate, jobsTabUri) {
  const header = `🏉🏉 รหัส ${batchCode} | ${formatBatchLabelDate(batchLabelDate)} 🏉🏉`;
  const body = jobs.map((j) => j.previewLine).join("\n\n");
  return buildLinkButtonMessage(`${header}\n\n${body}`, "กดรับงาน", jobsTabUri);
}
