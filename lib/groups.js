...
import { getGroupSummary, getGroupMemberIds } from "./line";
...
// ดึงรายชื่อสมาชิกกลุ่มจาก LINE (เฉพาะคนที่แอดเพื่อนบอทแล้ว) มาผูกเข้ากลุ่มให้หมด
// ใช้ตอนบอทเพิ่งถูกเชิญเข้ากลุ่ม เพื่อให้สมาชิกเดิมที่ยังไม่เคยพิมพ์อะไรเห็นข้อมูลกลุ่มได้เลย
export async function syncGroupMembers(group) {
  const memberIds = await getGroupMemberIds(group.line_group_id);
...
// เวลามีคนแอดเพื่อนบอทใหม่ เช็คทุกกลุ่มที่บอทอยู่ว่าเขาเป็นสมาชิกอยู่ด้วยมั้ย (LINE บอกได้เฉพาะคนที่แอดเพื่อนแล้ว)
// ถ้าใช่ก็ผูกให้ทันที ไม่ต้องรอให้เขาพิมพ์อะไรในกลุ่มก่อน
export async function linkUserToAllGroupsIfMember(userId, lineUserId) {
  const { data: groups } = await supabase.from("groups").select("id, line_group_id");
  for (const g of groups ?? []) {
    const memberIds = await getGroupMemberIds(g.line_group_id);
    if (memberIds.includes(lineUserId)) {
      await linkUserToGroup(userId, g.id, "member");
    }
  }
}
