...
import { getUnreadJobIds } from "./chat";
...
// งานที่ยังเปิดอยู่ เฉพาะกลุ่มที่ user สังกัดจริง (รวมงานที่ตัวเองโพสต์ด้วย เผื่อคนโพสต์อยากเห็น/ตรวจสอบงานที่ลงไปเอง)
...
export async function getUserHistory(userId) {
  ...
  const { data: claims } = await supabase
    .from("job_claims")
    .select(
      "id, claimed_at, released_at, job:job_id(id, detail, wage, payment_method, status, created_at, job_code, poster:poster_id(display_name), group:group_id(group_name))"
    )
  ...
  const jobIds = [
    ...(posted ?? []).map((j) => j.id),
    ...(claims ?? []).map((c) => c.job?.id),
  ];
  const unreadJobIds = await getUnreadJobIds(userId, jobIds);

  return { posted: posted ?? [], claimed: claims ?? [], unreadJobIds };
}
