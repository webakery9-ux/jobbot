...
// เอารหัสงานจากบรรทัดแรกของแต่ละบล็อกตรงๆ (เช่น "XH31 - JKJ984565" -> "XH31") ไม่ได้สร้างรหัสเอง
function extractJobCode(firstLine) {
  const token = (firstLine || "").trim().split(/\s+/)[0];
  return token || (firstLine || "").trim();
}
...
// rawText: ข้อความทั้งก้อนที่วางมา
// คืนค่า { jobs: [{ jobCode, detail, wage, vehicleType, previewLine }], errors: [{ blockIndex, preview, message }] }
export function parseBulkJobsText(rawText) {
  ...
  blocks.forEach((linesArr, idx) => {
    ...
    const firstLine = detailLines.find((l) => l.trim() !== "") || "";
    const jobCode = extractJobCode(firstLine);
